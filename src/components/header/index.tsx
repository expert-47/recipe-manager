...
function ResponsiveAppBar() {
  ...
  const menuHandler = (page: string) => {
    if (page === "Add Recipe") {
      router.push("/add-recipe");
    }
  };

  const pageNavigator = (pageName: string) => {
    if (pageName === "Profile") {
      router.push("/profile");
      ...
      <Container maxWidth="xl">
        <Toolbar disableGutters>
          ...
          {/* <Box sx={{ flexGrow: 1, display: { xs: "flex", md: "none" } }}>
            ...
            >
              {pages.map((page) => (
                <MenuItem key={page} onClick={() => menuHandler(page)}>
          ...
          >
            ...
            >
              {settings.map((setting) => (
                <MenuItem key={setting} onClick={() => pageNavigator(setting)}>
